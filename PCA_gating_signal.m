...
function [gating_signal2,W, gating_signal_test, gating_signal_test2] = PCA_gating_signal(ksp2,params)
...
data(isnan(data)==1) = 0; %pdh replace all NaN`s with zero


if params.visualize==1;
    figure(992); imshow(data,[]); title('input data for PCA'); xlabel('coil*slice dimension');ylabel('time (Fs)')
end
