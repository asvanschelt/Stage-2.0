function [ksp_sorted,k_sorted,gating_signal,indexphase, gating_signal_s] = ksp2frames(ksp2,k,params, gating_signal)
...
if ~isfield(params,'nBins'); params.nBins=10'; disp('Warning: no params.nBins, default:6');end
...
elseif nargin==3
    ...
    [gating_signal, gating_signal_test, gating_signal_test2] = generate_gating_signal(ksp2,params); %IF NO GATING SIGNAL PROVIDED; GENERATE
else
...
%     case 'v3'; p=7;
% end
% time_blok = params.time_blok;
% time_total = [0:2*time_blok:2*time_blok*angles-time_blok];
% difference = params.end_time - time_total(length(time_total));
% time_total = time_total(:)+difference;
% nBins = params.nBins;
% [F, peaks, mns] = SortInBins(gating_signal_s,time_total,nBins,1,p);
% 
% figure(110); hold on;
% E = zeros(nBins,1);
% C = hsv(nBins);
...
% axis([0 params.end_time -inf inf]);
% title('MRI Binning')
% for ii=1:nBins
%     B = F(:,6)==ii;
%     X = F(B,1);
%     T = time_total(B);
%     scatter(T,X,[],C(ii,:),'o');
%     E(ii,1)=length(find(F(:,6)==ii));
% end
% hold off

% figure(150); hold on;
% plot(time_total,F(:,1)*100,'g'); hold on;
...
if params.visualize==1
    ...
    %COMPARING
    if params.visualize==1
        cmap=hsv(params.nBins);
        figure(103);
        subplot(212);hold on;
        plot(gating_signal_s)
        title('MRI Binning from code')
        axis([0 length(gating_signal_s) -inf inf]);
        for i=1:params.nBins
            plot(indexphase(i,:),gating_signal_s(indexphase(i,:)),'MarkerFaceColor','none','MarkerEdgeColor',cmap(i,:),'Marker','*','Linestyle','none');end
        hold off
